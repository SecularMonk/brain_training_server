...
function generateStringsAndValues({ numItems = 3 }: { numItems?: number }) {
   try {
...
function selectRandomElement({ iterable }: { iterable: string }) {
   try {
...
function createProblemStatement({ initialisationData }: { initialisationData: ReturnType<typeof generateStringsAndValues> }) {
   try {
...
function getCorrectAnswer({ comparator, itemsAreEqual }: { comparator: string; itemsAreEqual: boolean }) {
   switch (comparator) {
      case "opposite":
         if (itemsAreEqual) {
            return "No";
